...
def test_scf(parser):
    ...
    assert pytest.approx(sec_run.x_octopus_input_Spacing, 0.28345892)
    ...
    assert pytest.approx(sec_system.atom_positions[1][0].magnitude, 1.91979671e-10)

    sec_scc = sec_run.section_single_configuration_calculation[0]
    assert pytest.approx(sec_scc.energy_total.magnitude, -6.91625667e-17)
    assert pytest.approx(sec_scc.energy_electrostatic.magnitude, 4.79087203e-18)
    ...
    assert pytest.approx(sec_eig.eigenvalues_values[0][4][6].magnitude, 5.26639723e-19)
    assert sec_eig.eigenvalues_occupation[0][16][1] == 2.0
    sec_scf = sec_scc.section_scf_iteration
    assert len(sec_scf) == 8
    assert pytest.approx(sec_scf[3].energy_total_scf_iteration.magnitude, -6.91495422e-17)
...
def test_spinpol(parser):
    ...
    assert pytest.approx(sec_scc.energy_reference_fermi[0].magnitude, 7.39160185e-19)
    sec_eig = sec_scc.section_eigenvalues[0]
    assert np.shape(sec_eig.eigenvalues_values) == (2, 10, 20)
    assert pytest.approx(sec_eig.eigenvalues_values[1][5][4].magnitude, -7.40576381e-18)
    ...
    assert pytest.approx(sec_scfs[0].energy_total_scf_iteration.magnitude, -1.02450582e-15)
    assert pytest.approx(sec_scfs[5].energy_reference_fermi_iteration[0].magnitude, 7.85685151e-19)
